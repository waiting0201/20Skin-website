...
Nuxt 3 純靜態（`nuxt generate`），預渲染約 950 頁。**後台是另一個套件**：[`apps/admin/`](../admin/)（Vite ＋ Vue 3 的 SPA），它的 build 產物寫進本專案的 `public/admin/`，隨 `nuxt generate` 一起打包。

⚠️ **建置有順序相依：先 admin 後 web** —— 反過來會打包到上一次的後台產物，而且不會有錯誤訊息：
...
> ⚠️ 後台在 [`apps/admin/`](../admin/)，**不受這四條管**。後台沒有設計稿，是依功能切的
> （docs/09 §8）—— 但它會沿用 `base.css` 的設計 token（`--brand`／`--ink`／`--line`⋯），
> 視覺才不會像兩個站。
...
- 後台（`apps/admin/`）第二輪的 13 個畫面：審核佇列、媒體庫、未命中題目、sitemap／301／匯出、
  首頁版位、選單、全站設定、帳號、角色（側邊選單已列成灰階佔位）
- ~~後台（`apps/admin/`）的上傳、富文本、拖曳排序~~ —— 上傳已改成向 API 取 SAS 的
  瀏覽器直傳（決策 9）、21 個區塊 JSON 欄位已改成表單（決策 17，⚠️ 段落仍是純文字，
  不做行內格式是 Tim 定案）、排序已在內容清單／首頁版位／導覽選單三處改成拖曳（2026-09-17）
