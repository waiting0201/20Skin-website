...
### ✅ 2026-09-18：必填沒填時，紅字在那一格下面，畫面也跳過去（決策 31）

Tim：「後台的驗證，如果有必填沒填，紅色警訊要顯示在那個欄位下，然後畫面要跳到那邊」。

**逐欄位的紅字原本就有**（`.adm-field__error`，規則在 `src/validation.ts`），缺的是另外兩件事：

**① 畫面不會跳過去。** 儲存按鈕是**釘在標題列上**的（2026-09-18 才改成 sticky），
按下去的那一刻出錯的欄位幾乎一定在畫面外，使用者看到的只有頂端一句
「有 3 個欄位需要修正」，然後得自己捲著找。新增 `src/scroll-to-error.ts`：
捲到位置最上面的那一格、把游標放進去。六個畫面共用：內容編輯（本文／SEO／排程三處）、
首頁版位編排、帳號管理、清單的「新增」、301 轉址、未命中題目建 FAQ 草稿。

🔴 **錨點是欄位外框上的 `data-error-key`，值＝錯誤鍵本身**（含 `bodyBlocks[3].image`
這種路徑）。新增欄位時忘了加不會壞掉，也不會有任何訊息 —— 只是那一格退回「捲不過去」
（`scroll-to-error.ts` 會逐段退到父層，至少帶到正確的區塊）。

🔴 **捲動要讓開兩層 sticky**（頂列 ＋ 編輯頁標題列），高度一律**現場量**，
理由同 `sticky-head.ts`：寫死的數字會讓欄位剛好停在標題列底下，
看起來就是「跳過去了，但什麼都沒有」。`focus()` 因此一定要帶 `preventScroll`。

**② 摺疊起來的區塊，紅字根本沒有渲染。** 這一個是真的 bug，不只是體感：
摺疊列是 `v-if` 不是 `v-show`（長文章唯一的效能措施），所以一篇文章的第 12 個區塊
少了必填欄位時，症狀是**頂端說有欄位沒填、整頁一個紅字都找不到**。
現在 `StructuredField.vue` 會依錯誤路徑把沿途每一列都展開再捲過去。

⚠️ **頂端那句總結留著**：它說的是「總共幾個要修」，而欄位旁邊的紅字一次只看得到一個。

驗收：`tools/admin-e2e/check.mjs` 多兩條（清空標題 → 捲到底 → 按儲存 → 紅字要在畫面裡、
不可被標題列蓋住、游標要在那一格；清空段落文字 → 全部收合 → 按儲存 → 那一列要自己展開）。
**`typecheck` 與 `build` 對這兩件事一律沉默** —— 捲動與展開只發生在真的瀏覽器裡。
2026-09-18 實跑 43 條全過。

### ✅ 2026-09-18：區塊 JSON 欄位只留表單，標題列的灰線不再黏著卡片

Tim：「後台表單中，只接顯示表單的部分就好，進階直接編輯 JSON 的部分可以不用，客戶不會用」
「有一些表單上方有一條灰線，跟下方白色表單區塊會黏在一起，像醫師表單那樣」。

**① 「表單／進階：直接編輯 JSON」兩顆切換鈕移除**（`StructuredField.vue`）。

🔴 **拿掉的是入口，不是原始模式本身。** 那個等寬 textarea **留著**，它仍會自己出現在兩種情況：
① 這個項目沒有 schema（`page` 的未知 slug，決策 17 刻意不硬套一份）；
② 既有資料 parse 不出來或最外層型別對不上（雙重編碼、手改壞的 JSON）。
那兩種情況下它是唯一看得到、也改得到那份資料的地方 —— 少了它，畫面會是一個空表單，
而使用者一按儲存就把原本的值蓋掉。第二種情況現在多一句說明（原本靠切換鈕自己去試）。

⚠️ 連帶：`toRawMode`／`toFormMode`／`switchError` 一起刪掉（切換時的 JSON 語法檢查
只服務那兩顆按鈕）；送出前的語法檢查在 `validation.ts`，**那支不能動** —— 上面兩種
情況送出的仍然是字串。

**② 編輯畫面標題列的分隔線改畫在 `::after` 上**（`admin.css` §14）。

原本是 `border-bottom: 1px`：線只有一個像素，線的正下方立刻就是白色卡片，
兩者黏成一條，看起來像卡片自己多了一道邊。
🔴 **修法不是 `margin-bottom`** —— 標題列是 sticky 的，margin 留在原本的位置、不跟著釘住，
一捲動卡片就又貼回線的下面。留白必須屬於標題列**自己的盒子**：
`padding-bottom` 改成 `sp-3 + sp-4`，線畫在 `::after`（`bottom: sp-4`）。
⚠️ 右欄的 sticky 頂端因此**不再另外加 sp-4**（那段留白已經算進 `--adm-page-head-h`），
加兩次右欄會比主欄低一截。

兩件事都加了 `tools/admin-e2e/check.mjs` 的回歸檢查（切換鈕不可以回來、`border-bottom`
不可以改回去）——`typecheck` 與 `build` 對這兩種改動一律沉默。

### ✅ 2026-09-18：首頁版位編排——35 次請求降到 3 次，右邊那片空白給了主視覺

Tim：「首頁版位編排頁很慢」「右側也是有一片空白」「表單再往上滑的時候，右上角的
發佈跟儲存按鈕也要停在右上」。

**慢在哪**（Playwright 實測，本機 API）：整頁 **35 次請求，其中 24 次打 `/admin/article`**。

| 來源 | 次數 | 為什麼 |
|---|---|---|
| 標題快取 | 14（文章 12） | 為了把 `ContentItemId` 顯示成標題，**把整個單元翻頁抓回來** —— 而 API 的 `HomeSectionItemDto.ContentTitle` 早就一起回來了 |
| 五個挑選器 | 19（文章 12） | `RelationPicker` 在 `onMounted` 就把整個目標單元抓回來，不管使用者會不會動它 |

改完 **3 次**（版位、首頁那筆 Page、找 Page 的清單各一次），`<option>` 由 1152 個降到
打開才載、每次最多 100 個。文章與標籤這種超過一頁的單元改出現**關鍵字搜尋框**。
🔴 **本機數字要乘上正式環境的倍率再看**：Azure SQL Basic（5 DTU）＋ 跨區往返，
24 次多餘的請求在那裡不是 0.3 秒的事。

**版面**：改回兩欄，右欄 400px 放**主視覺輪播**（Tim 指定）——它是七個版位裡唯一
「有東西可以編」的，其餘六個只有一個開關加一個挑選器。右欄**不 sticky**（它是最高的
那張卡片），也**不在拖曳清單裡**（沒有把手，排序值原封不動）。

**標題列釘在頂端**：`.adm-editor > .adm-page__head` sticky，九個內容模型的編輯頁一起適用。
🔴 右欄的 sticky 頂端要讓開它，而它的高度會變（標題換行、視窗寬度）——
`src/sticky-head.ts` 用 ResizeObserver 量出來寫進 `--adm-page-head-h`（實測 99px）。

🔴 **順手修掉一個靜默的 bug**：存檔送的是 `sections`（載入時的順序），而 `putSections`
用**陣列位置**當 `sortOrder` —— 拖曳只改了每一筆的 `sortOrder` 欄位，沒有動陣列順序，
所以**排好了、存下去卻還是原來的順序**，畫面上不會有任何異常。改送 `sortedSections`，
並以 Playwright 實測「拖 → 存 → 重載順序還在 → 拖回來」。

**同日再拿掉三個「編了不生效」的東西**（Tim 指定，與「版位標題／副標」那次同一條理由）：

| 拿掉的 | 為什麼 |
|---|---|
| 拖曳把手 | **前台根本不讀版位順序** —— `apps/web/app/pages/index.vue` 的七個 `<section>` 是寫死的先後，`sortOrder` 只有後台自己看得到 |
| 「在首頁顯示這個版位」 | 關掉**只讓那一區變空**，區塊本身照樣渲染（那個 `<section>` 沒有 `v-if`），前台會留下一塊只有標題與英文小標的空白區 —— 比關不掉更糟 |
| 圖片欄位的刪檔說明 | 兩句話互相矛盾：ImageField 說「會被刪」、版位這邊說「不會」。**後者才對**（版位設定沒走發布時清 blob 那條路）。Tim：「不用寫，只要有建議尺寸就好」 |

⚠️ `isEnabled` 與 `sortOrder` 的值**仍然照讀照送**，只是畫面上沒有入口能改。
正式與本機的七個版位**都是 enabled**（2026-09-18 實查兩邊），所以沒有「關掉了卻開不回來」的資料。
⚠️ 要讓那兩件事真的生效是**改前台**（照 `sortOrder` 排、`isEnabled=false` 就整個 `<section>` 不渲染），
不是把後台的控制項加回來。
⚠️ 連帶：`adminApi.site.home.reorderSections()` 沒有呼叫端了，已刪除（順序隨 `saveDraft()` 一起送）。

🔴 **做刪檔說明時踩到一個靜默的 Vue 陷阱**：boolean prop **缺席會被轉成 `false`**，不是 undefined。
第一版寫成 `props.deletesOldFile !== false` 當防呆，結果是**九個內容模型那邊每一個圖片欄位的
刪檔警告全部靜默消失**（那裡是真的會刪，警告不該消失），畫面上完全看不出來。
改用 `withDefaults(..., { deletesOldFile: true })`，並在 `check.mjs` 補了一條專門擋它的檢查。

閘門：`pnpm --filter admin typecheck`／`build`、`tools/admin-e2e/check.mjs` **38 項**全過
（新增五條：兩欄版面、沒有把手、沒有顯示開關、主視覺只給建議尺寸、內容模型的刪檔警告還在）；
另以 Playwright 量了請求數、兩欄寬度（712／400）、sticky 標題列（捲動後 y=60、按鈕點得到）、
編輯頁右欄沒有被標題列蓋住（y=175 ≥ 159），並實測「拖 → 存 → 重載順序還在」之後才拿掉把手。
...
### ✅ 八大困擾的科別圖示全部破圖（2026-09-18 Tim 回報，已修）

回報是「肌膚困擾的每個頁面，hero 裡 SKIN CONCERN 左邊那張圖都沒顯示」。
一查是**兩個模板共 16 張圖全破**：困擾細節頁的 hero 圖示（8 張，40×40）與
困擾總覽頁的八大入口圖示（8 張，64×64）——同一個欄位（`concern.icon`）餵的。

**根因**：`data/concerns.ts` 算圖片路徑用的是 <code>/assets/img/spec-${slug}.png</code>，
而設計稿交付的檔名是 `spec-01.png`…`spec-08.png`，**與 slug 無關**。
搬進資料庫之前（commit `6e74aa7` 以前）每一筆困擾各自寫著自己的 `spec-0N.png`，
那一次搬遷把它改成用 slug 拼 —— 於是每一個請求都去要一個**不存在的檔案**。

🔴 **這與 CLAUDE.md 決策 14 那個「拿 Blob 網址的檔名去算路徑」是同一類錯誤**：
算得出一個看起來很合理、但不存在的路徑，而 `typecheck`／`verify:css`／`build`
**沒有一個會有意見** —— 對建置來說那只是一個字串。`verify:links` 也抓不到：
它爬的是網頁之間的連結，不是頁面裡的 `<img>`。

**修法**：對照表回到 `data/_presentation.ts`（圖示是版面素材，決策 14），
`concernIconFor(slug, title)` 查不到就回 `null`，兩個模板改成 `v-if` ——
**不退回某一張預設圖**：掛別的科別的線條圖比沒有圖更糟。

驗證：本機前台對正式 API 實跑，8 個細節頁 ＋ 總覽頁共 16 個路徑全部 200，
Playwright 確認 `naturalWidth > 0`（真的畫出來了，不是只有 200）。

⚠️ 同型風險：全站只剩這一處用字串拼 `/assets/` 路徑（已 grep 確認），其餘都是字面值。

---

### ✅ 四處「後台編得動、前台不理它」（2026-09-18 發現並修掉）

起點是一個很小的問題：「頁尾的社群連結是不是沒有跟後台套好」。是沒有 —— 而且一查就發現
**同一類問題有四處**。共同特徵是：三個驗收閘（`typecheck`／`verify:css`／`build`）沒有一個抓得到，
因為程式完全正確，錯的是「它讀的不是那份資料」。

| 位置 | 症狀 | 修法 |
|---|---|---|
| 頁尾社群連結與版權文案 | 後台「導覽選單與頁尾」存得進 `footer.social.json`／`footer.copyright`，但 **`GET /site-settings/public` 根本沒回這兩個鍵**，前台 `navigation.ts` 寫死兩筆 Facebook、版權是字面值 | 白名單加兩個鍵 ＋ 前台改讀 ＋ migration `SeedFooterSocialLinks` 把原本寫死的兩筆搬進資料庫 |
| 標籤頁的分類 tab | 四顆寫死的 `<li>`（另外兩個 blog 列表元件都是 `v-for`）。後台在「分類與標籤」改名、調順序、新增都不會跟；分類被刪掉那顆會連到 404 | 改成 `getArticleCategories()` |
| 案例列表的兩排篩選 | 七顆按鈕裡**六顆連到 `#`**，按了毫無反應；名稱又與後台的困擾／療程分類各存一份。卡片標籤永遠是空的 —— 讀的是 `relationsOf(案例, 療程→困擾)`，而案例身上不可能有那個型別的關聯 | 兩排由實際案例推導（案例→對應療程→困擾／分類），改用查詢字串真的會篩（做法同醫師列表 09-16 那次）；選項少於兩個就整排不出現 |
| 「27 項療程」 | 療程 09-14 就變成 28 了。首頁 09-15 改成推導，但**療程總覽、404、搜尋頁三處漏掉**，而且那句話裡的分類順序也與畫面上的實際順序對不上 | 療程總覽全部改推導（含分類名稱），404 與搜尋頁改寫掉數字（那兩頁沒有療程資料，不值得為一個數字多打一次 API） |

⚠️ 連帶移除：案例列表 mockup 留下的**假分頁列**（1／2／3 全部連 `#`）。案例沒有分頁，
留著等於對訪客謊報「還有第 2、3 頁」。

🔴 **這一批真正的教訓是「哪一種錯誤沒有守門員」**：
`typecheck` 看型別、`verify:css` 看樣式、`verify:links` 看連結 —— 沒有一個看得到
「這個欄位的值有沒有被任何人讀過」。四處全部是這一類，而且全部在正式站上活了好幾天。
同型的前科：SEO 五欄（09-17）、首頁版位的標題／副標（09-18）、全站設定的 Logo 上傳（09-17）。

⚠️ **驗過的方式**：本機起 API ＋ 站台，直接改資料庫模擬後台編輯，確認頁尾三個連結
（Instagram／LINE／不認得的網域）各自套到對的圖示、版權文案跟著換；案例篩選以臨時的第二筆
案例驗過交集、`is-active`、網址亂填退回全部；`verify:links` 1863 頁全綠。

⚠️ **沒有一起修的**：`docs/06-page-inventory.md` 仍寫「27 項中有 12 項外連」（該數字自 09-14 起是 28），
以及全站還有 **4684 個 `href="#"` 佔位連結**（`verify:links` 實計，mockup 遺留）——
後者要逐頁判斷哪些是「還沒接的功能」、哪些是「本來就沒有目標」，不是一次能掃完的事。

---

### ✅ 選單不再是第二份資料（2026-09-18，決策 29）

承上一節：修完「頁尾社群沒接後台」之後往下查，發現選單的 **48 個節點全部是手打的名稱與路徑**，
而正式站已經因此分岔 —— 同一個 `skincare` 分類，「分類與標籤」叫**膚質改善**、選單叫**醫美保養**；
兩邊的排序也各是一份，所以 header 子選單的順序與療程總覽頁不一樣。
Tim 的指示是「後台的維護要依據前台的單元」，選了「子選單自動帶入單元 ＋ 其餘指向內容」。

| 層 | 改了什麼 |
|---|---|
| 資料庫 | `MenuItems.AutoChildren`（0 自己維護／1 困擾／2 療程分類／3 文章分類／4 據點）＋ `Label` 可留空。遷移 `MenuFollowsContentUnits` 把 46 個手打節點接成「指向內容」、名稱與標題一致的清空、四段子選單改自動 |
| API | `/menu` 展開自動子項目；名稱留空時用**已發布快照**的標題 |
| 後台 | 頂層列多一個「子項目」選擇；自動時收掉子項目編輯區並說明；名稱欄位顯示「留空＝跟著內容」與目前的自動名稱 |
| 前台 | **一行都沒改** —— 拿到的樹狀結構形狀不變 |

🔴 **兩個欄位的取值規則相反，而且都是刻意的**：名稱用**已發布快照**（在後台改了名還沒發布，
選單不會先變 —— 決策 14），排序用**即時值**（拖完立刻生效，與內容清單一致）。兩者都實測過。

⚠️ **可見的變化**：頁尾「肌膚困擾」那一欄原本是院方挑的 4 項，自動之後是全部 8 項。

🔴 **踩到的三個坑，全部是「編譯完全正常、執行期才炸或才錯」**：
① `SELECT 1 AS Source` 的 1 是 int，而收它的記錄宣告成 `byte` —— Dapper 在執行期丟
「找不到相符建構子」，`/menu` 整支 500，也就是**全站的選單**。要 `CAST(... AS tinyint)`。
② `MenuItems.Url` 與 `ContentItems.UrlPath` 定序不同（後者是網址用的 BIN2），
比對不寫 `COLLATE` 的話 SQL Server 直接拒絕執行（Msg 468）。
③ 本機用 `sqlcmd -Q` 改 `ContentItems` 會被 `QUOTED_IDENTIFIER` 擋下（Msg 1934），
而我把輸出丟進 /dev/null —— 於是「改了名但選單沒變」看起來像測試通過，其實 UPDATE 根本沒執行。
**驗證腳本不要吃掉錯誤輸出**，這一條與 web.yml 那段 `CODE=$(curl …)` 是同一種錯。

⚠️ 順帶修掉 `tools/admin-e2e/check.mjs` 兩條**過期的斷言**：它們比對的是
「從 Blob 刪除」，而 31b70b2（後台文案去技術化）已經把那句話改成中文說法。
一條因此永遠失敗、另一條（檢查「不該出現」）**永遠通過** —— 後者更危險。

✅ 驗證：`admin-e2e/check.mjs` 40 項全過（含實際登入後開選單頁目視）、`api-smoke/read` 34 項、
`verify:links` 1863 頁零斷鏈、前台 typecheck／verify:css／verify:seo-head 全過。

### ✅ 首頁的醫師與據點也不再是第二份順序（2026-09-18，決策 30）

承上一節：Tim 問「首頁各個版位的內容排序，是不是也跟真正的內容排序脫鉤了」。
查下去發現**比脫鉤更根本** —— 那兩個版位「挑選」的是**整個單元**
（實查：醫師 14/14、據點 2/2、品牌理念 1 筆；只有精選療程 4/28 與最新文章 4/1100 是真策展）。
也就是說它們不是策展，是同一批內容的第二份順序，與選單那次同一個形狀，只是分岔的是順序。
兩份目前還一致，所以症狀還沒出現 —— 在「內容 → 醫師」拖一次就會分岔，而兩邊都沒有任何徵兆。

| 層 | 改了什麼 |
|---|---|
| 資料庫 | **0 支 migration**。`HomeSectionItems` 那些列留著（切回手挑時還在），只是沒有人讀 |
| API | `GET /home` 對 `doctors`／`clinics` 改成算繪當下取整個單元（`AutoSections` ＋ `GetHomeAutoItemsAsync`），順序用該單元的 `SortOrder` |
| 後台 | 那兩張卡片沒有挑選器了，改成一句「要增減或調整順序請到○○清單，改完前台立刻就會變」 |
| 前台 | 只改註解與**據點卡片的比對鍵**（標題 → slug） |

🔴 **連帶修掉「標題用工作副本」**：`GET /home` 的項目標題與 `relations[].toTitle` 原本取
`ContentItems.Title`，等於「在後台改標題、還沒發布，首頁卡片與關聯卡片當場就換掉」。
三處（首頁、選單、關聯卡片）現在同一條規則：**標題取已發布快照、網址取即時值**。

🔴 **據點卡片改用 slug 比對**（`apps/web/app/data/home.ts`）。原本比對的是標題，
而標題是院方改得動的欄位 —— 一改就比不到，症狀是那張卡的**電話、地址、看診時段整組變空**，
沒有任何錯誤訊息。

✅ 驗證（本機，對本機資料庫）：
- 實跑「拖一次就跟」——用 `PUT /admin/doctor/sort` 對調前兩位醫師，`GET /home` **當場**換順序
  （沒有碰首頁、沒有重新發布），再對調回來還原；
- `/home` 與 `GET /doctor`、`GET /clinic` 的順序**逐筆相同**（14 筆、2 筆）；
- 前台首頁算繪出 14 位醫師、據點卡片電話／地址／時段表都在；困擾頁的建議療程卡片標題正常；
- `admin-e2e/check.mjs` **41 項全過**（含新增一條：醫師與據點沒有挑選器、精選療程與最新文章還有）；
- 前台 typecheck／verify:css／verify:seo-head、後台 typecheck、API build 全過。

⚠️ 沒驗到的一項：「在後台改標題但不發布，首頁不跟」只做了程式碼層面的比對
（與 `/menu` 同一段寫法，那一段 2026-09-18 實測過），沒有真的在本機改一筆標題來看 ——
那會把一筆已發布內容推回草稿狀態，不值得為了看一眼而弄髒開發庫。
