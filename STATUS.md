...
| **`/team/` 的兩排篩選是假的** | 🔴 專長篩選（皮膚疾病／雷射光電／注射微整／體態管理）是**照抄 mockup 的寫死值、`href="#"`，按下去沒有任何反應** —— 程式裡自己寫著「示意：正式站由後台專長 tag 自動產生，此處尚未串接篩選邏輯」。實際上後台有 **14 個** type 1 專長標籤。<br>⚠️ 旁邊「院區：」那排更誤導 —— 看起來是篩選，連到的卻是 `/clinics/siji/`（據點頁），按下去會**離開團隊列表**。<br>（2026-09-16 盤點發現，尚未決定要做成哪一種）|
