...
| **`verify:links`** | 🔴 **SSR 改版後這道閘失效了，目前是紅的。** 它掃的是建置產物裡的頁面 HTML，而 SSR 的產物沒有頁面 —— 2026-09-16 只掃到 2 頁、0 條連結。<br>⚠️ 我加了 `MIN_PAGES=50` 的失敗門檻，**因為它原本是「掃 0 個連結、全數通過」的假綠**。要恢復這道閘，腳本得改成對**執行中的站台**爬連結。<br>最後一次有效數字：掃 1848 頁、120289 條、零斷鏈（2026-09-15，靜態版） |
...
- 🔴 **SSR 分支上線前的擋路項：`/about/` 兩頁的 `UrlPath` 根因**（§八）。
  原本靠 `postbuild.mjs` 濾掉，而那層已隨第 3 段刪除 —— 現在 sitemap 又收了兩個 404 網址
- 🔴 **`verify:links` 要改成爬執行中的站台**，否則這道驗收閘在 SSR 之下永遠是紅的（詳見 §二的表）
...
| sitemap | 🔴 收了兩個 404 網址。**2026-09-16 起又收回去了** —— SSR 第 3 段把 sitemap 改成執行期產生，連帶刪掉 `postbuild.mjs` 那層過濾，治標的補丁跟著沒了。這是我在第 3 段造成的退步，如實記在這裡：**SSR 分支上線前必須先處理根因**（§八 的三個選項），不要再補一層過濾 |
| **站內搜尋** | 🔴 **使用者看得到的 bug**：搜「新中式美學」會給兩筆連到 `/new-chinese-aesthetics/` 與 `/makeup-style/` 的結果，點下去 404。改成 `GET /search` 之後**照舊**（2026-09-16 實測）—— 它讀的是同一個 `ci.UrlPath`，換資料來源救不了資料本身錯的欄位 |
