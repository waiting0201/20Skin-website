...
Tim：「後台的表單右邊會有一整區空白，把排程區跟刪除區移到右邊空白區」。

主欄壓在 960px（2026-09-17 收單欄時沿用原本主欄的量測寬度），而 `.adm-main`
實際可用 1216px —— 右邊固定空著 256px。現在切回兩欄：

| | |
|---|---|
| 外層 | `.adm-editor--aside` 把寬度上限由 960px 放寬到 `--adm-content-max`（1280px） |
| 版面 | `.adm-editor-layout`（grid，`minmax(0,1fr)` ＋ 300px，gap sp-5，**`align-items: start`**） |
| 右欄 | `.adm-editor-side`：sticky，top 讓開 topbar；排程卡片＋「儲存排程」為一組，危險區為另一組 |
| 窄螢幕 | ≤**1264**px 收回單欄、右欄回到 SEO 之後（＝ 09-17 的順序） |

⚠️ 主欄因此由 960px 收到約 890px（960＋24＋300 ＞ 1216）。是刻意的取捨：右欄再窄就
放不下 `datetime-local`，而主欄兩欄 field-grid 各少 35px 沒有影響。
⚠️ `align-items: start` 不可省 —— 預設的 stretch 會把右欄拉成與主欄等高，
sticky 在「已經和捲動容器一樣高」的元素上完全不會作用（看起來就像沒寫）。
⚠️ 1264 這個斷點是算出來的：可用寬度 ＝ 視窗 − 側欄 240 − 內距 64，要 > 960（主欄上限）
才會有「右邊那一條空白」。**視窗 ≤1264 本來就沒有空白可以填**，切兩欄只會把主欄壓到 500px。

🔴 **代價：刪除不再「要捲到底才看得到」**（那是 09-17 刻意的防護）。剩下的三層要一起在：
與儲存（綠）／發布（藍）不同色、不同寬（右欄的刪除**不拉滿寬**，靠右維持 `btn--danger` 線框紅）、
隔一整段距離（`.adm-editor-side` 的 gap 用 sp-6，比卡片之間更寬），加上 `window.confirm`。
🔴 **回來的是版面，不是工作流** —— 右欄沒有送審、審核、版本歷程，`.adm-workflow` 沒有復活。

⚠️ 首頁版位編排（`HomeSections.vue`）**維持單欄 960px**：它沒有排程也沒有刪除，
右欄會是空的。`.adm-editor` 的預設值因此沒有動，放寬是 `--aside` 這個修飾子的事。

閘門：`pnpm --filter admin build`（含 `vue-tsc`）／`tools/admin-e2e/check.mjs`。
...
**✅ 正式環境已還原**（2026-09-17，Tim 從瀏覽器提供 token）：
工作表與快照都壞了，版本歷程 4 版裡 v4／v3 壞、**v2 是最後一版好的**。

🔴 **沒有整筆還原 v2 —— 那會退掉別的東西。** 動手前比對 v2 與 v4：
「精選療程」的順序被人重排過（`[1140,1149,1156,1163]` → `[1156,1140,1149,1163]`），
首頁那筆 Page 的 `SortOrder` 也因為 `NormalizeSortOrder` 由 1 變成 0。
整筆 restore 會把這兩件都寫回去。改成**只把 hero 與 specialties 兩欄的 settings
搬回來**，其餘沿用現況。

覆核：`GET /home` 兩欄都是陣列；前台首頁 36113 bytes、八大專科 ×4、Blob 圖片 ×34
（壞掉時是 2 與 22）、精選療程仍是重排後的 `1156,1140,1149,1163`、瀏覽器 console 乾淨、
主視覺輪播四張都在。
