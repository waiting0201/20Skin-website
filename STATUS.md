...
✅ **2026-09-14：`Redirects` 表已有 782 列**（780 條遷移工具產生 ＋ 2 條系統自動）。
...
2026-09-14 匯入 **780 條**（`Source=1` 遷移工具產生）：
709 條主站文章內頁 ＋ 71 條 blog slug 被正規化造成的站內位移。
加上原有 2 條系統自動規則，現為 **782 列、全部啟用、0 條已人工核對**。

✅ **「文章內頁的舊網址型態尚未確認」已解決。**
實地確認是 **`share_info.php?no=NNN`**，不是先前假設的 `share.php?id=N` ——
那份合成佔位資料的**來源型態本來就是錯的**，還好沒有匯入。
`tools/legacy-import/import.mjs` 會依實際抓到的 709 篇產出 `redirects.csv`
（另含 71 條 blog slug 改寫造成的站內位移），走 `POST /admin/redirect/import` 匯入。

⚠️ 仍然不完整：**只有文章內頁**。列表頁（`share.php?class=…&page=N`、
約 40 條中文 query 的年份組合）與療程頁（`product*.php`）還沒盤點，
docs/06 §6 的「約 770 條是下限，孤兒頁面尚未盤點」依然成立。
