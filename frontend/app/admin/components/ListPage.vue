...
import { can, canCreateTerm } from '~/admin/permissions'
import type { AdminRecord, ContentStatus, UnitKey } from '~/admin/types'
import { STATUS_LABEL } from '~/admin/types'
...
async function createDraft(overrideFields: Record<string, unknown> = {}) {
  ...
  try {
    ...
    await router.push(`/admin/${props.unit}/${created.id}`)
  } catch (e) {
            ...
            <tr v-for="(item, idx) in items" :key="item.id">
              ...
              <td v-for="col in def.listColumns" :key="col.key" :class="{ 'is-wrap': col.key === 'title' }">
                <span v-if="col.key === 'title'" class="adm-table__title">
                  <NuxtLink :to="`/admin/${unit}/${item.id}`">{{ item.title }}</NuxtLink>
                </span>
