...
// 真正擋得住的是 API 端（見 app/admin/permissions.ts 檔頭註解）。
import { computed, nextTick, onBeforeUnmount, onMounted, reactive, ref, watch } from 'vue'
import { onBeforeRouteLeave, useRouter } from 'vue-router'
import { adminApi, ApiError } from '~/admin/api/client'
import { currentUser } from '~/admin/auth'
import { can, canDeleteTerm, ownsRecord } from '~/admin/permissions'
import type { AdminRecord, RelationItem, SeoMeta, UnitKey } from '~/admin/types'
import { emptySeo } from '~/admin/types'
import type { UnitField } from '~/admin/unit-schema'
...
const canDelete = computed(() => {
  ...
  // 權限碼——docs/10-api.md §3.3 的逐單元例外，見 app/admin/permissions.ts 檔頭。
              ...
              <div class="adm-field-grid">
                ...
                >
                  ...
                  <input
                    ...
                    class="adm-input"
                    :type="field.type"
                    :value="bodyForm.fields[field.key] ?? ''"
                  ...
                  <HoursEditor
                    ...
                    :model-value="(bodyForm.fields[field.key] as { dayOfWeek: number; startTime: string; endTime: string }[]) ?? []"
                    @update:model-value="(v) => (bodyForm.fields[field.key] = v)"
                  />
