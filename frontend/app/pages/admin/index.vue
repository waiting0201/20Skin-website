...
import { adminApi, type DashboardSummary } from '~/admin/api/client'
            ...
            <table class="adm-table">
              ...
              <tbody>
                <tr v-for="unit in unitKeys" :key="unit">
                  <td><NuxtLink :to="`/admin/${unit}`">{{ UNIT_REGISTRY[unit].label }}</NuxtLink></td>
            ...
            <div v-for="item in summary.myRejected" :key="item.id" class="adm-list-item">
              <div>
                <div class="adm-list-item__title">
                  <NuxtLink :to="`/admin/${item.unit}/${item.contentItemId}`">{{ item.title }}</NuxtLink>
                </div>
