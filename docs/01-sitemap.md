...
> 🔴 **是 391 篇，不是 101 篇。** 101 來自 2026-07-29 讀 Yoast `post-sitemap.xml`，
> 2026-09-14 以 WP REST 的 `x-wp-total` 實數確認為 391，其中只有 19 篇是 07-29 之後才發佈的
> —— 當時就數錯了。見 [06-page-inventory.md](06-page-inventory.md) §4。
>
> ⚠️ **「保留原 slug」有 71 篇做不到逐字保留。** API 只收 `^[a-z0-9]+(-[a-z0-9]+)*$`，
> 而 WP 有 68 篇用底線、1 篇全形底線、1 篇 U+2010 連字號、2 篇純中文。
> 這些改寫成連字號（純中文的改用 `post-{id}`），並**補一條站內 301**
> （`/blog/atopic_dermatitis_2022/` → `/blog/atopic-dermatitis-2022/`），
> 所以院方只要做單純的網域轉址，讀者仍會走到正確的頁面，只多一跳。
> **解法刻意不是放寬 API** —— 底線在 Google 的斷詞裡不算分隔符，
