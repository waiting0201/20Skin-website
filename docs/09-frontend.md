...
`/search/` 是 21 個模板之一，但沒有伺服器端搜尋服務。做法：建置期由 `content/*.json` 產生一份精簡索引（標題、摘要、AI 摘要、分類、URL），前端載入後以 client 端比對。

⚠️ **索引檔會隨 800 篇文章成長，要設上限。** 全文進索引會變成好幾 MB 的下載；只放標題與摘要欄位。若日後不夠用，再評估外部搜尋服務 —— 不要為此改成 SSR。
...
31 個畫面（[06](06-page-inventory.md) §5），全部在 `/admin/**`，`ssr: false`。
