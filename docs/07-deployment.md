...
> 🔴 **代價要寫清楚。** 後台路徑為 `/admin/`（客戶指定，2026-08-10 定案），與舊站相同、是攻擊者第一個會試的路徑，**不提供任何隱蔽性**。原規劃的三道防線（雙因素、IP 白名單、登入次數限制）現在只剩兩道，登入端點直接暴露在公網掃描下。因此剩下兩項**必須做紮實，不能再打折**：
>
> - **雙因素驗證**：管理員與審核者強制開啟，不提供「記住此裝置」以外的略過方式
> - **登入次數限制**：需**帳號與來源 IP 雙維度**計數（只鎖帳號擋不住撞庫、只鎖 IP 擋不住分散式嘗試），鎖定事件即時觸發告警通知信（**不留存紀錄** —— 不做操作日誌，見 [08-database.md](08-database.md) §I）
> - 兩項細節見 [02-backend-cms.md](02-backend-cms.md) §4
>
...
| [02-backend-cms.md](02-backend-cms.md) | 排程發布改用 **Timer trigger**（§4，已非 GitHub Actions cron）；**後台 IP 白名單不做**，登入防護剩雙因素與次數限制兩項（§2）；角色授權在 Function 內驗證；800 篇匯入仍用本機腳本；語料匯出在建置期；媒體庫上傳為瀏覽器直傳 Blob（§3） |
