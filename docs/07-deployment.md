...
> - **帳密成為唯一憑證**，建議訂定密碼強度與輪替規則（[02-backend-cms.md](02-backend-cms.md) §4）。
>   種子密碼 `Admin@123` **不視為上線阻斷項**（院方判定，2026-09-16，不要重新提案）
...
1. ~~**排程發布改用 Timer trigger。**~~ 🔴 **2026-09-16：連 Timer 都不需要了。**
   ...
   ⚠️ 後台文案已於 2026-09-16 同步改成「上線時間／到這個時間點，前台就會看得到」——
   原本寫的「最早生效時間、到點後仍需一次全站重建才會上線」是靜態版的事實，
   留著等於**低估**現在的行為，而且那句話已經是錯的。
