...
因此以下一律不建：AI 對話紀錄（2026-09-18 AI 問答上線後**仍然不建**，理由改為個資，見 §I）、FAQ 的「已推送／待推送」狀態、巢狀分類的 `ParentId`、預約與購物相關的任何東西。
...
🔴 **後台因此不把「適應症」「禁忌症與注意事項」設成必填**（2026-09-18；困擾的「症狀描述」同理，8 筆裡 7 筆是空的）——必填的檢查跑在上傳與送出之前，於是那 27 筆連換一張封面圖都存不了，而紅字指的是另一個欄位，看起來像是圖片上傳壞掉。欄位本身在資料庫與 API 都允許 null，那道門檻是後台自己加的。
...
| AI 對話紀錄 | 🔴 **2026-09-18 AI 問答做了（CLAUDE.md 決策 28），但這張表仍然不建。** 舊理由「介接方式未定」已作廢，新理由是個資：醫療提問屬敏感，存下來就有保管責任。<br>⚠️ 連帶的代價要知情：**無法逐則抽查回答品質**。替代做法是 Application Insights 的結構化事件（命中率、延遲、被引用的內容），**不記問題與回答全文** —— 它不是資料表，也不含個資。<br>⚠️ 索引狀態同理不進資料庫：存在 Blob 的 manifest 裡，整套 **0 支 migration**。 |
