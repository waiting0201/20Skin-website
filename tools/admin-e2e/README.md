...
| 腳本 | 驗什麼 | 安全性 |
|---|---|---|
| `check.mjs` | 畫面開不開得起來、21 個區塊 JSON 欄位有沒有渲染成表單、摺疊列讀不讀得懂、追蹤碼擋不擋得住、模式切換的選中狀態、拖曳把手在不在（**首頁版位那一頁是「不該在」**）、編輯頁回不回得去列表、首頁版位的兩欄版面與那三個被拿掉的控制項、**必填沒填時紅字在不在那一格下面、畫面有沒有跳過去、摺疊起來的區塊有沒有自己展開**（決策 31） | 🟢 **不寫任何資料**（必填那一段會動表單，但驗證先擋下來，一個寫入請求都不會送出） |
