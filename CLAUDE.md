...
| 後台畫面 | **29** | 9 個內容模型 ＋ 首頁版位 ＋ 導覽選單。**不做操作日誌**、**不做媒體庫**（皆 2026-09-11 定案）、**不做審核佇列**（2026-09-17，決策 20）。32、31、30 都是舊數字 |
...
10. **前後台同一個 SWA、同一個網域**：`20skin.tw` 前台、**`20skin.tw/admin` 後台**（Nuxt `ssr: false` 的 SPA）。API 則在另一個網域 `api.20skin.tw`，見第 7 條。
   ...
   **種子密碼 `Admin@123` 不視為上線阻斷項**（Tim 判定，2026-09-16，**不要再提案更換**）——舊敘述「種子密碼上線前必須更換」已作廢。剩下的防線是 reCAPTCHA v3 與登入次數限制。
   ⚠️ **後台帳號管理頁那塊「安全防線只剩登入次數限制」的常駐紅色警示已移除**（Tim 指定 2026-09-17，**不要再加回來**）——
   它等於每次開帳號管理都在重提「換掉種子密碼」，而那件事已經定案不做；事實本身記在這裡與 docs/02 §4、docs/10 §5，那才是它該待的地方。
   **「首登強制改密碼」也不做了**（Tim 指定 2026-09-17：「密碼設定好就好，管理者不用再另設」）——
   管理者填的那一組就是最終密碼，`AppRouter` 那道 403 閘與登入頁那一關一起移除（**只拿掉一邊會讓舊資料的帳號登得進來卻每支端點都 403**）。
   **密碼長度下限由 8 改為 6**，`AccountHandler.MinPasswordLength` 與 `apps/admin/src/validation.ts` 的 `MIN_PASSWORD_LENGTH` 要一起改。
...
16. **後台的權限判斷以 API 發下來的 `permissions[]` 為準**，2026-09-12 定案。
   ...
   權限碼一律用 docs/08 §A-2 的 **28 列**（`content.{unit}.edit` 這種），
...
17. **內容欄位的「區塊 JSON」在後台一律是表單，不是讓人手打 JSON**（2026-09-17）。
   ...
   **首頁主視覺輪播**）
   ...
   🔴 **「表單／進階：直接編輯 JSON」的切換鈕 2026-09-18 移除**（Tim 指定：「只接顯示表單的
   部分就好，進階直接編輯 JSON 客戶不會用」）。**拿掉的是入口，不是原始模式本身** ——
   那個 textarea 仍會自己出現在兩種情況：沒有 schema（上一段那些頁面）、以及既有資料
   parse 不出來或最外層型別對不上。**不要跟著把 textarea 拿掉**：那兩種情況下它是唯一
   看得到也改得到那份資料的地方，少了它畫面會是一個空表單，一按儲存就把原值蓋掉。
   ⚠️ `validation.ts` 對原始模式的 JSON 語法檢查**要留著**，那兩種情況送出的仍然是字串。
   🔴 **SEO 區塊的「結構化資料覆寫（進階，JSON）」同理，2026-09-18 起預設不顯示**
   （Tim 指定：「JSON 客戶肯定不會輸入」）。它與上面那些「編了不生效」的控制項**不同** ——
   它是真的會生效的：填了就**整段取代**那一頁自動產生的 MedicalWebPage 與麵包屑
   （`usePageHead` 的覆寫分支）。所以拿掉的仍然只是入口：
   **這一筆已經有值時 textarea 照樣出現**（`EditPage.vue` 的 `showStructuredData`），
   否則既有的覆寫會變成看得到症狀、卻找不到也改不掉來源。
   🔴 那個判斷綁 `record`（伺服器版本）**不可以綁 `seoForm`** —— 綁表單的話，
   使用者把內容刪光的那一刻整格欄位就消失，人還在編輯中。清空後按儲存才收起來。
   ⚠️ 它**做不成表單**：JSON-LD 的欄位由 `@type` 決定，宣告不出一份 schema；
   而該有的結構化資料 21 個模板本來就各自自動產生了。零筆資料有值（2026-09-18 實查）。
...
19. **九個內容模型的編輯畫面只有「草稿」與「已發布」兩個狀態**（Tim 定案 2026-09-17：
   「送審太複雜，只需要草稿跟發佈就好」）。右側工作流側欄整個移除：
   **發布／取消發布在標題列**、排程與刪除在**右欄**。
   ⚠️ **版面 2026-09-18 再調一次**（Tim 指定「表單右邊會有一整區空白」）：排程與危險區
   由頁尾搬回右欄（`.adm-editor-layout` ＋ `.adm-editor-side`，sticky，300px）——
   主欄壓在 960px 之後右邊本來就空著一整條。
   🔴 **搬回來的是版面，不是工作流**：右欄沒有送審、沒有審核、沒有版本歷程，
   `.adm-workflow` 那組樣式也沒有復活，**不要往裡面加東西**。
   🔴 刪除因此**不再「要捲到底才看得到」**。防護改成三件事一起在：與儲存（綠）、
   發布（藍）不同色、不同寬、隔一整段距離（右欄的 gap 比卡片之間更寬），
   加上送出前的 `window.confirm`。少任何一件都會變回「差一格就刪掉一整筆內容」。
   ⚠️ **標題列 2026-09-18 起釘在頂端**（Tim 指定：「不要跟著被滑走」）——
   `.adm-editor > .adm-page__head` 是 sticky，發布與儲存捲不走。
   🔴 那個 `>` 不可省：`.adm-page__head` 在首頁版位那一頁還被當成**卡片內**的標題列用，
   少了直接子選擇器，七張卡片的標題會各自變成 sticky。
   🔴 右欄的 sticky 頂端要讓開它，而標題列的高度會變（標題換行、視窗寬度）——
   由 `src/sticky-head.ts` 量出來寫進 `--adm-page-head-h`，**不要改回寫死的數字**。
   ⚠️ **拿掉的是這個畫面的入口，不是機制** —— `POST /admin/{unit}/{id}/submit`、
   `ContentReviews`、審核佇列畫面與**首頁版位的送審**都還在（理由同「版本歷程」那次：
   端點留著沒有害處，而既有卡在「審核中」的資料仍然要有辦法放行）。
   🔴 **`canEditBody` 的「狀態 2 鎖定本文」不可以跟著拿掉** —— API 對審核中的內容
   一律拒絕更新，前端放行只會換來一個看不懂的 409。
   🔴 **代價**：沒有 `{unit}.publish` 的角色（內容編輯）在畫面上已經無法把內容送上線，
   要由有發布權的人打開那一筆按「發布」。見 [docs/02-backend-cms.md](docs/02-backend-cms.md) §4。
   ⚠️ 連帶：兩顆儲存按鈕靠右、移到白底卡片外、改成綠色 —— 原本的品牌藍與區塊 JSON
   欄位的「表單／進階 JSON」切換鈕同色，分不出哪一顆是存檔。**發布維持藍色**。
   ⚠️ 那組切換鈕 2026-09-18 已移除（決策 17），但**儲存不要改回藍色**：現在的分工是
   「綠＝留在後台、藍＝對外」，而那兩顆就並排在標題列上。
   ⚠️ 標題列與下方卡片之間那條分隔線畫在 `::after` 上，線的下面還有一段留白算在標題列
   自己的 `padding-bottom` 裡（Tim 指出 2026-09-18「灰線跟白色表單區塊黏在一起」）。
   🔴 **不可以改回 `border-bottom`，也不可以改用 `margin-bottom`** —— 標題列是 sticky 的，
   margin 不跟著釘住，一捲動卡片就又貼回線的下面。連帶右欄的 sticky 頂端不再另外加 sp-4。

20. **送審與審核者整層不做**（Tim 指定 2026-09-17）：「審核者跟審核佇列都不用，簡單一點」。
   範圍是**畫面、端點、權限碼、角色四層一起拿掉**，不是只藏畫面：
   審核佇列畫面與儀表板的「待審核／我的退件」、`POST /admin/{unit}/{id}/submit`、
   `GET|POST /admin/review/*`（含 `ReviewHandler`、`ReviewReadService`）、
   `content.submit`／`review.approve`／`review.reject` 三個權限碼（**31 → 28**）、
   「審核者」角色（**5 → 4**），以及首頁版位那一套自己的送審／核准／退回。
   🔴 **「內容編輯」因此拿到九個 `content.{unit}.publish`**（Tim 選定）——
   握有 publish 的角色只有審核者，不移交的話**全院只剩超級管理員能讓任何內容上線**。
   ⚠️ 這等於**刻意放棄「發布權與編輯權分離」**，docs/02 §4、docs/10 §4 的舊敘述已作廢。
   醫療廣告法規相關敘述請以主管機關函釋及院方法務意見為準。
   ⚠️ **為什麼端點不能像「版本歷程」那樣只拿掉畫面**：送審會把內容推進 `Status = InReview`，
   而那個狀態下 API 一律拒絕更新 —— 沒有核准端點的話，內容會卡死在**編不了也上不了線**的狀態。
   既有卡在該狀態的資料仍救得回來（`PATCH .../publish` 不檢查來源狀態）。
   ⚠️ **權限碼的 Id 沒有往前補，19–21 留成空號。** `RolePermissions` 是後台畫面改得動的資料，
   renumber 會讓「原本指著 22（`seo.edit`）」的手動列**靜默變成** `page.legal.edit`。
   ⚠️ `ContentReviews` 表與 `ContentStatus.InReview` **保留**（沒有人再寫入），拆掉要一支 migration。
   ⚠️ 高風險字詞只剩編輯器的即時提示，**伺服器端不再重掃**（原本那一次發生在送審）。

21. **清單的拖曳排序＝當頁內重新分配**（2026-09-17）。
   ⚠️ 適用的是**內容清單與導覽選單兩個畫面**——首頁版位編排的把手 2026-09-18 拿掉了（見決策 23）。送出的是「當頁那幾筆**原本佔住的
   那幾個排序值**」的重新分配，不是整個單元的順序，也不是陣列索引。因此與單元大小無關
   （分類與標籤 406 筆、案例都拖得動），篩選中也成立；代價是**不能跨頁搬移**。
   🔴 **前提是排序值不重複** —— migration `NormalizeSortOrder` 把每個型別重編成 0..n-1
   （值就是現在算出來的順序，**顯示順序不變**），`ContentHandler.CreateAsync` 也改成新增時
   取 `MAX(SortOrder)+1`；少了後者，正規化之後每一筆新內容都會插到整個單元的最前面。
   ⚠️ `adminApi.content.sort()` 的 `sortOrder` 由呼叫端決定 —— 退回用陣列索引的話，
   在第 3 頁拖一次就會把那 20 筆整批洗到最前面，而畫面上看起來完全正常。
   🔴 **文章不給拖**（`listSortable: false`，Tim 定案）：前台 `/blog/`、分類頁、標籤頁一律
   `sort=latest`（發布日期新到舊），`SortOrder` 只影響「某位醫師的文章」那一區 ——
   給把手等於給一個前台看不到效果的假功能。**不要因為「其他單元都有」就加回去。**
22. **後台清單與編輯頁的往返行為**（2026-09-17）：編輯頁的「回到清單」回到**進入時**的
   頁碼與篩選（`src/list-state.ts` 模組層記憶 ＋ 返回連結帶 `?restore=1`）。
   🔴 **還原要在 setup 這一層、任何 watcher 註冊之前做完**，不可以放進 `onMounted` ——
   那支 `immediate: true` 的記憶 watcher 會在 setup 當下先用新實例的預設值蓋掉記憶。
   ⚠️ 不放網址列是刻意的：這份記憶只服務「編輯頁 → 按返回」那一段來回，不跨越重新整理。
   （原本的理由「重新整理就是登出」已因決策 25 失效，但結論不變 ——
   重整之後這份記憶是空的，畫面回到預設清單，那是預期行為。）

25. **後台重新整理不再登出**（Tim 定案 2026-09-18）：**refresh token 放 `sessionStorage`**，
   開頁時先用它換回一組憑證與身分（`main.ts` 的 `restoreSession()`），換完才掛載 app。
   **access token 仍然只放記憶體**，那一顆寫進任何瀏覽器儲存都不行。
   🔴 **是 `sessionStorage` 不是 `localStorage`** —— 分頁關掉就沒了，
   換到的是「重整不登出」而不是「這台機器永久登入」。
   🔴 **還原必須在 `mount` 之前 await 完**：路由守衛只看 `isAuthenticated()`，
   換發還沒回來就掛載＝每次重整先閃一下登入頁。
   🔴 **`/auth/logout` 要把 refresh token 送進 body**（原本送的是 `{}`，等於空操作）——
   憑證會被持久化之後，「登出沒有真的撤銷」才開始有後果。連帶：登出要**先同步清掉
   `sessionStorage` 那一份**，再打撤銷端點（記憶體那份得留著，那支端點要 Bearer）。
   ⚠️ 代價：一次 XSS 可以拿走 refresh token（30 天），而後台沒有 IP 白名單也沒有雙因素
   （決策 10）。接得住的只剩後端的輪替 —— 舊 token 一被重用就撤銷該使用者全部憑證。
   ⚠️ 換發端點回的是完整的 `TokenResponse`（含 `permissions[]`），所以還原**不需要**另一支
   `/auth/me`；但**權限一定要跟著更新**，只還原 token 會讓剛被改掉角色權限的人繼續看到舊畫面。
   ⚠️ 已知邊角：**複製分頁會連 `sessionStorage` 一起複製**，兩邊各自換發會觸發「重用即全撤」。
   ⚠️ docs/09 §8、`src/api/http.ts`、`src/auth.ts` 舊敘述「重新整理分頁就會登出，這是設計行為」
   **已作廢**，不要照著它把 token 改回純記憶體。
...
23. **首頁主視覺輪播（hero banner）改由後台維護**（Tim 指定 2026-09-17）。
   ...
   ⚠️ **2026-09-18 起它自己佔右欄**（Tim 指定：右邊那一整片空白拿來放主視覺）——
   主欄是其餘六個版位，右欄 400px 放輪播（`.adm-editor-side--hero`）。
   ⚠️ 那六個裡真的還有挑選器的只剩三個（精選療程、最新文章、品牌理念摘要）——
   醫師與據點改成自動列出全部（決策 30），八大專科入口的挑選器早已拿掉（本條上方）。
   🔴 右欄**不 sticky**：它是這一頁最高的卡片，sticky 會把下半截釘在畫面外。
   🔴 **輪播圖的圖片節點帶 `deletesOldFile: false`**：版位設定的圖換掉**不會**被刪
   （沒有走內容模型那條「發布時清掉上一版獨有 blob」的路），少了這個旗標，
   `ImageField` 會顯示一句與事實相反的警告。畫面上**不寫刪不刪**，只留建議尺寸
   （Tim 指定：「不用寫，只要有建議尺寸就好」）。

   🔴 **這一頁 2026-09-18 再拿掉兩個「編了不生效」的東西**（Tim 指定，與決策 21、
   與「版位標題／副標」那次同一條理由）：
   ① **拖曳把手** —— 前台**根本不讀版位順序**：`apps/web/app/pages/index.vue` 的七個
   `<section>` 是寫死的先後，`sortOrder` 只有後台自己看得到；
   ② **「在首頁顯示這個版位」核取方塊** —— 關掉**只會讓那一區變空**，區塊本身照樣渲染
   （那個 `<section>` 沒有 `v-if`），前台會留下一塊只有標題與英文小標的空白區，比關掉更糟。
   ⚠️ **兩個欄位的值都還照讀照送**（`isEnabled`／`sortOrder` 原值回去），只是畫面上沒有入口能改。
   正式與本機的七個版位目前**都是 enabled**（2026-09-18 實查），所以沒有「關掉了卻開不回來」的資料。
   ⚠️ 要讓它們真的生效，是**改前台**（照 `sortOrder` 排、`isEnabled=false` 就整個 `<section>` 不渲染），
   不是把後台的控制項加回來。

27. **後台文案不放工程原理，也不放內部風險備註**（Tim 指定 2026-09-18：
   「後台還有沒有像這種太過技術的欄位跟說明呢」，接續決策 17 那一次的判準）。
   三條規則：
   ① **程式與資料庫的名字不出現在畫面上** —— `IsSystemLocked`、`NOT NULL`、
   `CaseImages`／`DoctorSchedules`、`canCreateTerm()`、`term.edit`、`409 CONFLICT_STATE`、
   `Blob`、首頁版位的 `sectionKey` 全部換成人話；
   ② **「為什麼要這樣設計」不寫在後台** —— `staticwebapp.config.json` 的 20 KB 上限、
   catch-all 路由、Application Insights、`X-Robots-Tag` 這些屬於 CLAUDE.md 與 docs/，
   後台只留「你在這個表單裡會踩到什麼」（例：不可用萬用字元、帶 `#` 的網址建不了規則）；
   ③ **內部風險備註不常駐在客戶畫面上** —— 「設定類不留痕」「沒有雙因素」這類事實
   記在文件裡，同帳號管理那塊紅色警示的處理（決策 10）。
   ⚠️ **英文術語不是一律禁止**：`Slug`／`Canonical`／`Meta Description` 改成
   「網址代稱（Slug）」這種中文在前、原字在括號裡的寫法 —— 院方要對外查資料、
   要跟工程端講同一件事時，那個字仍然得認得。GA4／GTM 這種產品名照留。
   🔴 **這一次真正要記住的不是措辭，是三處「文案沒跟上決策」**：
   角色頁寫「五個角色」（決策 20 已剩四個）、首頁版位寫「這裡只能開關與調整順序」
   （決策 23 兩個入口都拿掉了）、sitemap 設定寫「建置期算出來」（決策 6 起是執行期）。
   **文案是決策的第四個副本**（前三個是程式、CLAUDE.md、docs/），改決策時最容易漏掉它。
   🔴 最嚴重的那一個甚至不只是文案：角色頁的規則檢查寫死「內容編輯不該有發布權」，
   於是**對正確的設定顯示紅色警告並要人移除** —— 照做就是全院只剩超級管理員能發布
   （決策 20 刻意把發布權移交給內容編輯）。已改成反過來檢查「內容編輯少了哪幾種發布權」。
...
26. **後台的挑選器選項「打開才載」，而且只載第一頁**（2026-09-18）。
   `RelationPicker` 原本在 `onMounted` 就把**整個目標單元**抓回來
   （`taxonomy.unitOptions()` 會翻頁抓完），而使用者十次有九次不會動那個欄位。
   🔴 **首頁版位編排實測 35 次請求，其中 24 次是文章**（1100 筆 ÷ 100 ＝ 12 頁，載兩次：
   一次配標題、一次給挑選器），改完剩 **3 次**。
   🔴 **版位項目的標題 API 早就一起回來了**（`HomeSectionItemDto.ContentTitle`）——
   舊寫法丟掉它，再拿 id 去把整個單元抓回來配標題。**不要再建那種標題快取。**
   ⚠️ 超過一頁（文章 1100、標籤 406）時挑選器改出現**關鍵字搜尋框**
...
29. **選單跟著內容單元走，不存第二份**（Tim 指定 2026-09-18：「後台的維護要依據前台的單元」）。
   在此之前選單的 **48 個節點全部是手打的名稱與路徑**（`LinkKind=2`），等於每個療程分類、
   困擾、據點在資料庫裡都有兩份。🔴 **正式站已經分岔過**：同一個 `skincare` 分類，
   「分類與標籤」叫「膚質改善」、選單叫「醫美保養」；兩邊的排序也各是一份，所以 header
   的子選單順序與療程總覽頁不一樣 —— 那正是「排序也不對」的來源，不是排序壞掉。
   兩個機制（`MenuItems.AutoChildren` ＋ 空的 `Label`）：
   ① **名稱留空＝顯示被指到那筆內容的標題**，填了就是覆寫（頁尾「關於 20SKIN」指向標題為
      「品牌理念」的那一頁）。`LinkKind=2／3` 不可留空 —— 沒有可以跟的對象。
   ② **子選單可設「自動列出全部○○」**（困擾／療程分類／文章分類／據點四種），
      設了之後名稱、網址、順序都跟著單元，後台那一段不再逐項維護。
   🔴 **名稱取「已發布快照」的標題，不是 `ContentItems.Title`** —— 後者是工作副本，
   用它等於「在後台改了名稱、還沒發布，全站每一頁的選單當場就換掉」（決策 14 的同一條）。
   **排序相反，用即時值**，與內容清單的拖曳排序一致（拖完立刻生效）。
   ⚠️ **自動帶入的來源條件與其他公開讀取一致**：可見性 ＋ `UrlPath IS NOT NULL`。
   ⚠️ **切成自動之後自存的子項目列不刪**（migration 也不刪）：CI 是先遷移後部署，
   中間那一段是新 schema 配舊程式，刪了會讓前台子選單短暫消失；而且切回「自己維護」時還在。
   ⚠️ **已知的可見變化**：頁尾「肌膚困擾」原本是院方挑的 4 項，自動之後是全部 8 項。
   ⚠️ 自動帶入**只能掛在頂層**（選單只有兩層，第二層展開會長出第三層）。
   ⚠️ **`MenuItems.Url` 與 `ContentItems.UrlPath` 的定序不同**（後者是 `Latin1_General_100_BIN2`，
   網址做位元組比對）—— 兩者比對一定要明寫 `COLLATE`，少了它 SQL Server 直接拒絕執行（Msg 468）。
   見 [docs/08-database.md](docs/08-database.md) §G-3、[docs/02-backend-cms.md](docs/02-backend-cms.md) §3

30. **首頁的「醫師團隊」與「據點資訊」自動列出全部，不再逐筆挑選**（Tim 指定 2026-09-18）。
   問題與決策 29 的選單同一個形狀，只是分岔的東西不同：那兩個版位**挑的本來就是整個單元**
   （正式資料醫師 14/14、據點 2/2），所以它們不是策展，是同一批內容的**第二份順序** ——
   在「內容 → 醫師」拖一次排序，首頁不會跟，而且**兩邊都沒有任何徵兆**。
   現在名單與順序都由該單元決定（`PublicContentHandler.AutoSections`，條件與其他公開讀取
   一致：可見性 ＋ `UrlPath IS NOT NULL`，順序用 `ContentItems.SortOrder`）。
   🔴 **順序用即時值，所以內容清單拖完立刻生效，不必回首頁按發布** —— 與選單的自動子項目一致
   （決策 29），也與決策 14「結構性欄位用即時值」同一條。
   🔴 **精選療程（4/28）與最新文章（4/1100）維持手挑**，品牌理念摘要指向單一頁面也維持 ——
   那三個是真的策展，順序自己一份是對的。**不要因為「其他版位都自動了」就一起改掉。**
   ⚠️ 自動之後那兩個版位**自存的 `HomeSectionItems` 不刪**（migration 也不必寫，整條 0 支）：
   切回手挑時還在，理由同決策 29。後台那兩張卡片因此沒有挑選器，只剩一句「要增減或
   調整順序請到○○清單」。
   🔴 **連帶修掉「標題用工作副本」** —— `GET /home` 的項目標題與 `relations[].toTitle`
   原本取 `ContentItems.Title`，等於「在後台改了標題、還沒發布，首頁卡片與關聯卡片
   當場就換掉」（決策 14 明列的災難，選單 2026-09-18 才修過一次）。三處現在同一條規則：
   **標題取已發布快照、網址取即時值**。
   ⚠️ 前台據點卡片改用 **slug** 比對 `getClinics()`（原本比對標題）——
   標題是院方改得動的欄位，一改就比不到，而症狀是那張卡的電話、地址、看診時段
   **整組變空**，沒有任何錯誤訊息。
   見 [docs/02-backend-cms.md](docs/02-backend-cms.md) §3、[docs/08-database.md](docs/08-database.md) §G-2、[docs/10-api.md](docs/10-api.md) §2

31. **後台的必填驗證：紅字在那一格下面，而且畫面要跳過去**（Tim 指定 2026-09-18）。
   兩件事一起才算數 —— 只有紅字＝使用者找不到，只有捲動＝到了也不知道錯在哪。
   `apps/admin/src/scroll-to-error.ts` 負責「帶過去」，紅字仍由各欄位自己的
   `.adm-field__error` 渲染（`role="alert"`，讀屏軟體靠它）。
   🔴 **錨點是欄位外框上的 `data-error-key`，值＝錯誤鍵本身**（含 `bodyBlocks[3].image`
   這種路徑）。新增欄位時忘了加**不會壞掉，也不會有任何訊息**，只是那一格退回「捲不過去」。
   🔴 **摺疊起來的區塊必須先自己展開**（`StructuredField.vue` 的 watch）——
   摺疊列是 `v-if` 不是 `v-show`（長文章唯一的效能措施），收起來的那一列裡面的紅字
   **根本沒有渲染**，症狀是「頂端說有欄位沒填，整頁卻一個紅字都找不到」。
   ⚠️ 捲動要讓開**兩層 sticky**（頂列 ＋ 編輯頁標題列），高度一律現場量，理由同決策 19
   的 `--adm-page-head-h`：寫死的數字會讓欄位剛好停在標題列底下，看起來像「跳過去了
   但什麼都沒有」。`focus()` 也因此一定要帶 `preventScroll`。
   ⚠️ **頂端那句總結不要拿掉**：它說的是「總共幾個欄位要修」，而欄位旁邊的紅字一次
   只看得到一個。
   ⚠️ 由 `node tools/admin-e2e/check.mjs` 把關（typecheck 與 build 都看不到捲動與展開）。
